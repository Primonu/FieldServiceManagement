...
function InventoryPage() {
  ...
  const updateStock = (sku: string) => navigate(`/Manager/Inventory/UpdateStock?sku=${encodeURIComponent(sku)}`);

  return (
    <div className="min-vh-100 bg-light d-flex" onClick={() => setMenuSku(null)}>
      <Sidebar activeItem="Parts & Inventory" />
      <main
        className="min-vh-100 p-3 p-md-4 main-area"
        style={{ marginLeft: sidebarExpanded ? '232px' : '72px', transition: 'margin-left 0.2s ease' }}
      >
        <header className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
          ...
          <div className="d-flex flex-wrap gap-2">
            <button className="btn btn-outline-primary" onClick={() => navigate('/Manager/Inventory/UpdateStock')}><i className="bi bi-arrow-repeat me-2" />Update Stock</button>
            <button className="btn btn-primary" onClick={() => navigate('/Manager/Inventory/AddPart')}><i className="bi bi-plus-lg me-2" />Add New Part</button>
          </div>
        </header>
        ...
        <section className="card border-0 shadow-sm">
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle mb-0">
              <thead className="table-light"><tr><th className="text-uppercase small">SKU</th><th className="text-uppercase small">Part Name</th><th className="text-uppercase small">Unit Cost</th><th className="text-uppercase small">Stock Qty</th><th className="text-uppercase small">Status</th><th className="text-uppercase small text-end">Actions</th></tr></thead>
              <tbody>
                {loading ? <tr><td colSpan={6} className="text-center text-secondary py-5"><span className="spinner-border spinner-border-sm me-2" />Loading inventory...</td></tr> : error ? <tr><td colSpan={6} className="text-center text-danger py-5">{error}</td></tr> : visibleParts.length ? visibleParts.map((part) => (
                  <tr key={part.sku}>
                    ...
                    <td className="text-end position-relative">
                      <button className="btn btn-sm btn-outline-primary me-1" onClick={(event) => { event.stopPropagation(); updateStock(part.sku); }}><i className="bi bi-arrow-repeat me-1" />Update Stock</button>
                      <button className="btn btn-sm btn-light" onClick={(event) => { event.stopPropagation(); setMenuSku(menuSku === part.sku ? null : part.sku); }} aria-label={`Actions for ${part.name}`}><i className="bi bi-three-dots" /></button>
                      {menuSku === part.sku && <div className="dropdown-menu show position-absolute end-0 top-100 shadow-sm" onClick={(event) => event.stopPropagation()}><button className="dropdown-item" onClick={() => updateStock(part.sku)}><i className="bi bi-arrow-repeat me-2" />Update</button><button className="dropdown-item"><i className="bi bi-pencil me-2" />Edit</button><button className="dropdown-item text-danger"><i className="bi bi-trash3 me-2" />Delete</button></div>}
                    </td>
                  </tr>
