...
function Workorder(){
  ...
  const getPriorityColorClass = (priority: string): string =>{
    ...
    return(
        <div className="app-shell d-flex"style={{ display: "flex", minHeight: "100vh" }} onClick={() => setMenuId(null)}>
          <Sidebar activeItem='Work Orders'/>
        <main className="main main-area"style={{ flex: "1 1 auto", width: "auto", minWidth: 0, marginLeft: 0 }}>
        <header className="page-header">
          ...
          <button
            ...
            onClick={() => navigate("/Manager/CreateWork")}
          >
                      ...
                      <td className="action-cell">
                        ...
                        {menuId === wo.workCode && (
                          ...
                          style={{zIndex:9999,display:"block",top:"100%"}}>
                            <button onClick={()=> navigate(`/Manager/WorkDetails/${encodeURIComponent(wo.workCode)}`)} aria-label={`View details for ${wo.workCode}`}>
                              <i className="bi bi-eye" /> View
                            </button>
                            <button onClick={()=> navigate(`/Manager/AssignWorkOrder/${encodeURIComponent(wo.workCode)}`)} aria-label={`Assign work order ${wo.workCode}`}>
                              <i className="bi bi-cancel" /> Assign
                            </button>
                            <button><i className="bi bi-trash3" /> Delete</button>
                          </div>
                        )}
                      </td>
                    </tr>
